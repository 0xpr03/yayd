...
impl<'a> Converter<'a> {
    ...
    fn get_file_info(&self, video_file: &Path) -> Result<FileInfo,Error> {
        ...
        if let Some(cap) = regex_duration.captures(&stdout) {
            ...
            Err(Error::FFMPEGError(format!("Couldn't get max frames: {}",stdout)))
        }
    }
    ...
    /// ffmpeg uses \r as \n
    fn run_merge_cmd(&self, audio_file: &Path, video_file: &Path, output_file: &Path) -> Result<Child,Error> {
    ...
    fn run_audio_extract_to_mp3(&self, video_file: &Path, output_file: &Path) -> Result<Child, Error> {
        let mut command = self.create_ffmpeg_base("ffmpeg");
        command.args(&["-threads", "0"]);
        command.arg("-i");
        command.arg(video_file);
