use mysql::conn::pool::{self,MyPool};
...
pub fn clear_query_states(pool: &pool::MyPool) {
    let affected = try_return!(pool.prep_exec("UPDATE `querydetails` SET `code` = ?, `status` = NULL WHERE `code` = ? OR `code` = ?",(CODE_FAILED_INTERNAL,CODE_STARTED, CODE_IN_PROGRESS))).affected_rows();
    if affected != 0 {
...
pub fn set_query_code(pool: & MyPool, code: &i8, qid: &i64) { // same here
...
pub fn add_file_entry(pool: & MyPool, fid: &i64, name: &str, real_name: &str) -> Result<(), Error> {
    trace!("name: {}",name);
...
pub fn request_entry<'a>(pool: &'a MyPool) -> Option<Request<'a>> {
    let mut stmt = try_reoption!(pool.prepare("SELECT queries.qid,url,quality,zip,`from`,`to` FROM querydetails \
                    INNER JOIN queries \
    ...
    if playlist {
        ...
        compress = true;
        from = from_value::<Option<i32>>(result[4].clone()).unwrap();
        to = from_value::<Option<i32>>(result[5].clone()).unwrap();
    } else {
        from = 0;
        to = 0;
        compress = false;
    }
    let request = Request { url: from_value::<String>(result[1].clone()),
                                    quality: from_value::<i16>(result[2].clone()),
                                    qid: from_value::<i64>(result[0].clone()),
                                    internal_id: from_value::<i64>(result[0].clone()),
                                    pool: pool,
                                    playlist: playlist,
                                    compress: compress,
                                    to: to,
